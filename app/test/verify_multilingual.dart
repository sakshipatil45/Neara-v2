...
void main() {
  test('Verify Multilingual AI Logic', () async {
    ...
    await Future.delayed(const Duration(seconds: 20));
    await testVoiceInterpretation(service, "There is a fire in the kitchen! Help!");
  }, timeout: const Timeout(Duration(minutes: 5)));
}

Future<void> testRequest(GeminiService service, String transcript, String lang) async {
  try {
...
Future<void> testVoiceInterpretation(GeminiService service, String transcript) async {
  int retries = 0;
  while (retries < 3) {
    try {
      print('\nInput: "$transcript"');
