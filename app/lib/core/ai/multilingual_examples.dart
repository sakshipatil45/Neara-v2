...
class _Example4MultipleLanguagesState
  ...
  Future<void> _processRequest() async {
    await ref
        .read(multilingualAssistantProvider.notifier)
        .processRequest(_controller.text, selectedLanguage: _selectedLanguage);
  }
...
class Example8ErrorHandling extends ConsumerWidget {
  ...
  Future<void> _processWithErrorHandling(
    ...
    try {
      await ref
          .read(multilingualAssistantProvider.notifier)
          .processRequest('test request', selectedLanguage: 'en');
    } catch (e) {
...
class Example10CustomHandler extends ConsumerWidget {
  ...
  void _handleUrgentRequest(
    BuildContext context,
    MultilingualResponse response,
  ) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  void _showNormalResponse(
    BuildContext context,
    MultilingualResponse response,
  ) {
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(response.responseText)));
  }
